...
import { UserFilter } from '@/services/user-management-service';
...
export function UserFilterBar({ onSearch, isLoading }: UserFilterBarProps) {
    ...
    >
      <div className="flex-1 min-w-[200px]">
        <Input
          ...
          onChange={(e) => setFilter({ ...filter, userCode: e.target.value })}
      ...
      <div className="w-[180px]">
        <select
          value={filter.userStatus?.toString() || ''}
          onChange={(e) => {
            const val = e.target.value;
            setFilter({ ...filter, userStatus: val ? Number(val) : undefined });
          }}
